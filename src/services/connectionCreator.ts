import { ConnectionConfig, ConnectionMetadata, ConnectionPoints } from '../types/plugin';
...
export class ConnectionCreator {
  ...
  async createConnection(frame1: FrameNode, frame2: FrameNode, config: ConnectionConfig): Promise<GroupNode> {
    const connectionPoints = this.pathCalculator.calculateConnectionPoints(frame1, frame2, config);
    
    const line = this.createMainLine(frame1, frame2, connectionPoints, config);
    const elements: SceneNode[] = [line];

    // Create arrow heads
    const { endAngle, startAngle } = this.calculateArrowAngles(connectionPoints, config);
    
    const endArrowHead = this.arrowRenderer.createArrowHead(connectionPoints.endPoint, endAngle, config);
    if (endArrowHead) {
      endArrowHead.name = 'End Arrow Head';
      elements.push(endArrowHead);
    }

    const startArrowHead = this.arrowRenderer.createStartArrowHead(connectionPoints.startPoint, startAngle, config);
    if (startArrowHead) {
    ...
    // Create label
    const labelFrame = await this.createConnectionLabel(connectionPoints, config);
  ...
  private calculateArrowAngles(connectionPoints: ConnectionPoints, config: ConnectionConfig) {
  ...
  private async createConnectionLabel(connectionPoints: ConnectionPoints, config: ConnectionConfig) {
