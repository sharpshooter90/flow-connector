...
export class ConnectionUpdater {
  ...
  async updateConnection(
    ...
    const connection = figma.currentPage.findOne(
    ...
    // Use provided frames or fall back to original frames
    const frame1 =
      newFrame1 ||
      (figma.currentPage.findOne(
        (node) => node.id === metadata.frame1Id
      ) as FrameNode);
    const frame2 =
      newFrame2 ||
      (figma.currentPage.findOne(
        (node) => node.id === metadata.frame2Id
      ) as FrameNode);
  ...
  async checkAndUpdateConnections(autoUpdateEnabled: boolean) {
    ...
    for (const [connectionId, metadata] of trackedConnections) {
      ...
      const frame1 = figma.currentPage.findOne(
        (node) => node.id === metadata.frame1Id
      ) as FrameNode;
      const frame2 = figma.currentPage.findOne(
        (node) => node.id === metadata.frame2Id
      ) as FrameNode;
      ...
      const currentConnectionPoints =
        this.pathCalculator.calculateConnectionPoints(
          frame1,
          frame2,
          metadata.config
        );
      const shouldUpdate = this.connectionNeedsUpdate(
        connection,
    ...
    for (const { connection, metadata } of connectionsToUpdate) {
      try {
        ...
        if (frame1 && frame2) {
          ...
          this.connectionManager.addTrackedConnection(
            newConnection.id,
            metadata
          );

          const wasSelected = currentSelection.some(
            (node) => node.id === connection.id
          );
          if (wasSelected) {
  ...
  private connectionNeedsUpdate(
    connection: GroupNode,
    newConnectionPoints: ConnectionPoints,
    config: ConnectionConfig
  ): boolean {
    const line = connection.children.find((child) =>
      child.name.startsWith("Connection:")
    ) as VectorNode;
    ...
    const pathMatch = pathData.match(
      /M\s*([\d.-]+)\s*([\d.-]+).*?(?:L|C).*?([\d.-]+)\s*([\d.-]+)(?:\s|$)/
    );
    ...
    const threshold = 1;
    const startMoved =
      Math.abs(currentStart.x - newStart.x) > threshold ||
